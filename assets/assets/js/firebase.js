...
function GetAllListing() {
  ...
  get(child(dbref, "food")).then((snapshot) => {
    if (snapshot.exists()) {
      ...
      let placholder = `<center><a href="/assets/AddFood.html"><i class="fa fa-plus fa-5x" aria-hidden="true"></i></a></center>`;
      document
        .querySelector("#part2")
...
function AddFood() {
  ...
  set(ref(db, "food" + "/" + uniqueId), {
    ...
    .then(() => {
      ...
      window.location.href = "/assets/listing.html";
    })
    .catch((err) => alert("error" + err));
}
...
(async () => {
  if (window.location.pathname == "/assets/listing.html") {
...
(async () => {
  if (window.location.pathname == "/assets/UserProfile.html") {
    const rawData = sessionStorage.getItem("userData");
    if (rawData) {
      const userData = await JSON.parse(rawData);
...
(async () => {
  if (window.location.pathname == "/assets/map.html") {
    const rawData = sessionStorage.getItem("userData");
    if (rawData) {
      const userData = await JSON.parse(rawData);
...
(async () => {
  if (window.location.pathname == "/assets/history.html") {
    const rawData = sessionStorage.getItem("userData");
    if (rawData) {
      const userData = await JSON.parse(rawData);
      if(userData.type!="volunteer")
...
(async () => {
  if (window.location.pathname == "/assets/AddFood.html") {
    const rawData = sessionStorage.getItem("userData");
    if (rawData) {
      const userData = await JSON.parse(rawData);
      if(userData.type!="volunteer")
